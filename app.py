...
def login():
    if request.method == 'GET' and 'next' in request.args:
    ...
    if request.method == 'POST':
        ...
        user = next((u for u in users if u['username'] == username and u['password'] == hashed_pw), None)
        if user:
...
def signup():
    ...
    if request.method == 'POST':
        ...
        hashed_pw = hash_password(password)
        otp = str(random.randint(100000, 999999))
        session['pending_signup'] = {'username': username, 'email': enc_email, 'password': hashed_pw}
        session['otp'] = otp
        session['show_otp_alert'] = True
...
def delete_account():
    ...
    users = load_users()
    user = next((u for u in users if u['username'] == session['username']), None)
    email = decrypt_email(user['email']) if user else ''
    # If MetaMask user, show confirmation only
    if user and user.get('metamask'):
        if request.method == 'POST':
            users = [u for u in users if u['username'] != user['username']]
            save_users(users)
            session.clear()
            flash('MetaMask account deleted successfully!', 'success')
            return redirect(url_for('login'))
        return render_template('delete_account_metamask.html', metamask=user['metamask'])
    # Normal user: password/OTP flow
    if request.method == 'POST':
        password = request.form['password']
        if user and user['password'] == hash_password(password):
            ...
            session['otp'] = otp
            session['show_otp_alert'] = True
            session['otp_type'] = 'delete_account'
            session['delete_email'] = user['email']
            return redirect(url_for('otp'))
        else:
            flash('Incorrect password!', 'danger')
    return render_template('delete_account.html', email=email)
